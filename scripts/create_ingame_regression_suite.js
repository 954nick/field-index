...
const headWorkerPath = path.join(root, "scripts", "create_head_regression_suite.js");
...
function getCachedHeadCatalogState() {
    try {
        ...
        return {
            exists: true,
            total: heads.length,
            usableUnique: usable.filter(entry => entry.head_type === "unique").length,
            usableGeneric: usable.filter(entry => entry.head_type === "generic").length
        };
    } catch {
        return { exists: false, total: 0, usableUnique: 0, usableGeneric: 0 };
    }
}

function cachedHeadCatalogIsUsable() {
    const state = getCachedHeadCatalogState();
    return state.usableUnique >= 1 && state.usableGeneric >= 1;
}
...
function runHeadPhase(source, options = {}) {
    const reportPath = path.join(dataDirectory, "ingame_regression_heads.json");
    const args = [headWorkerPath, source, "--report-path", reportPath];
    if (options.outputDirectory) args.push("--output-dir", options.outputDirectory);
    ...
    if (result.status !== 0) {
        throw new Error(`heads regression phase failed with exit code ${result.status ?? "unknown"}`);
    }
    return JSON.parse(fs.readFileSync(reportPath, "utf8"));
}
...
async function main() {
    ...
    if (!options.skipHeadTests) {
        const cacheState = getCachedHeadCatalogState();
        // Once a catalog has been explicitly prepared, regression generation reuses it.
        // Incomplete entries remain incomplete instead of triggering another multi-minute
        // scan of the same save. Users can explicitly rerun mapping:prepare when desired.
        const useCache = options.noHeadAutoBuild || cacheState.total > 0;
        if (useCache) {
            console.log(
                `\nUsing cached Head ID catalog (${cacheState.total} entries; ` +
                `${cacheState.usableUnique} usable unique / ${cacheState.usableGeneric} usable generic); ` +
                "full Head scan is not repeated."
            );
        }
        reports.push(runHeadPhase(sourcePath, { outputDirectory }));
    }
...
export { cachedHeadCatalogIsUsable, getCachedHeadCatalogState, mergeReports, parseArgs };
